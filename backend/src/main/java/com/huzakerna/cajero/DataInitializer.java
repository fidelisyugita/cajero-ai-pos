    ...
    public void run(String... args) throws Exception {
        if (sRepo.count() == 0) {
            ...
            List<MeasureUnit> measureUnits = List.of(
                    MeasureUnit.builder().code("PCS").name("Pieces").description("Individual items").build(),
                    MeasureUnit.builder().code("G").name("Gram").description("Weight measurement").build(),
                    MeasureUnit.builder().code("ML").name("Mililiter").description("Volume measurement").build(),
                    MeasureUnit.builder().code("MM").name("Milimeter").description("Length measurement").build());
            muRepo.saveAll(measureUnits);

            List<ProductCategory> productCategories = List.of(
                    ProductCategory.builder().code("FOOD").name("Food").description("Food items").storeId(storeId)
                            .build(),
                    ProductCategory.builder().code("SNACK").name("Snack").description("Snack items").storeId(storeId)
                            .build(),
                    ProductCategory.builder().code("DRINK").name("Drink").description("Drink items").storeId(storeId)
                            .build());
            ...
            List<TransactionStatus> transactionStatuses = List.of(
                    TransactionStatus.builder().code("PENDING").name("Pending")
                            ...
                            .build(),
                    TransactionStatus.builder().code("COMPLETED").name("Completed").description("Transaction completed")
                            .build(),
                    TransactionStatus.builder().code("CANCELLED").name("Cancelled").description("Transaction cancelled")
                            .build(),
                    TransactionStatus.builder().code("REFUNDED").name("Refunded").description("Transaction refunded")
                            .build());
