  ...
  return (
    <View style={$.container}>
      <View style={$.header}>
        ...
        <Button size="sm" title={t("remove_all")} variant="link" onPress={clearOrder} />
      </View>
      <FlashList
        data={items}
...
const Footer = () => {
  ...
  const globalTax = useOrderStore((_state) => 0);
  ...
  const totalItemTax = items.reduce((sum, item) => sum + (item.tax || 0), 0);
  ...
  const renderItem = (label: string, value: string) => {
    return (
      <View style={$.summaryItem}>
        <Text adjustsFontSizeToFit style={$.summaryLabel}>
          {label}
        </Text>
        <Text style={$.summaryValue} numberOfLines={1} adjustsFontSizeToFit>
          {value}
        </Text>
      </View>
    );
      ...
      <View style={$.summaryColumn}>
        ...
        <View style={$.summaryCard}>
          <View style={$.summaryTop}>
            ...
            {finalDiscount > 0 && renderItem(t("discount"), `- ${formatCurrency(finalDiscount)}`)}
          ...
          <View style={[$.summaryBottom, $.summaryItem]}>
            <Text adjustsFontSizeToFit style={$.totalLabelText}>
              {t("total")}
            </Text>
            <Text style={$.totalValueText} numberOfLines={1} adjustsFontSizeToFit>
