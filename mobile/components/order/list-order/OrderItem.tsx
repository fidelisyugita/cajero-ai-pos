...
const OrderItem = ({ item, isExpanded, onToggle, onRemove, onEdit }: OrderItemProps) => {
  ...
  const discountStr = discountVal > 0 ? `- ${formatCurrency(discountVal)}` : null;
  const finalPrice = subtotal - discountVal;

  return (
    <View style={[$.container, isExpanded && $.expandedContainer]}>
      <TouchableOpacity style={$.header} onPress={onToggle} activeOpacity={0.7}>
        <View style={$.left}>
          <Entypo name={isExpanded ? "chevron-down" : "chevron-right"} size={vs(20)} color="#666" />
            ...
            <View style={$.variantsList}>
              {item.variants.map((v) => (
                <View key={`${v.groupName}-${v.name}`} style={$.variantRow}>
                  ...
                  {v.price > 0 && <Text style={$.variantPrice}>{formatCurrency(v.price)}</Text>}
                </View>
              ))}
            </View>
          )}
