...
const Header = ({ children }: HeaderProps) => {
  ...
  }, [transactionTypes.length, fetchAll]);

  return (
    <View style={$.container}>
      <View style={$.leftContainer}>{children}</View>
      <Profile />
    </View>
  );
};
  ...
  return (
    <View style={$.profileContainer}>
      ...
      <View style={$.profileInfoContainer}>
        <Text adjustsFontSizeToFit style={$.profileName}>
          {user?.name || t("guest")}
        </Text>
        <Text adjustsFontSizeToFit style={$.profilePosition}>
          {user?.roleCode || t("visitor")}
        </Text>
      </View>
    </View>
  );
