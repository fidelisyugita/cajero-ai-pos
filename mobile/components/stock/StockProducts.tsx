...
const StockProducts = ({ searchQuery = "" }: StockProductsProps) => {
    ...
    return (
        <View style={$.container}>
            ...
            <FlashList<Product>
                contentContainerStyle={[$.listContent, sortedProducts.length === 0 && { flex: 1 }]}
                data={sortedProducts}
                ...
                renderItem={({ item }: { item: Product }) => (
                    <StockRow
                        item={item}
                        onHistoryPress={() => setSelectedProduct(item)}
                        onEditPress={() =>
                            router.push({
                ...
                ListEmptyComponent={
                    <EmptyState
                        ...
                        subtitle={t("empty_products_subtitle")}
                    />
                }
            />
            {selectedProduct && (
            ...
            {editingProduct && (
                <StockUpdateModal
                    ...
                    isLoading={updateProductStockMutation.isPending}
                />
            )}
        </View>
    );
...
const StockRow = memo(({ item, onHistoryPress, onEditPress, onStockUpdatePress }: { item: Product; onHistoryPress: () => void; onEditPress: () => void; onStockUpdatePress: () => void }) => {
    ...
    return (
        <View style={$.row}>
            <Text style={[$.cellText, { flex: COLUMNS[0].flex }]} numberOfLines={1}>{item.name}</Text>
            <Text style={[$.cellText, { flex: COLUMNS[1].flex }]} numberOfLines={1}>{item.categoryName || item.categoryCode || '-'}</Text>
