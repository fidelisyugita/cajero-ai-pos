...
export const useMeasureUnitStore = create<MeasureUnitStore>((set, get) => ({
  ...
  // Actions
  selectMeasureUnit: (measureUnit) => set({ selectedMeasureUnit: measureUnit }),
  ...
  setNewMeasureUnitName: (name: string) => set({ newMeasureUnitName: name }),

  setNewMeasureUnitCode: (code: string) => set({ newMeasureUnitCode: code }),
