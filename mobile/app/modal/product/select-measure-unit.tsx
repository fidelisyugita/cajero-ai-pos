...
const AddMeasureUnit = () => {
  const { newMeasureUnitName, setNewMeasureUnitName, newMeasureUnitCode, setNewMeasureUnitCode } =
    useMeasureUnitStore();
...
const MeasureUnitItem = memo(({ item }: MeasureUnitItemProps) => {
  const checked = useMeasureUnitStore((s) => s.selectedMeasureUnit?.code === item.code);
  ...
  return (
    <View style={$.measureUnitItemContainer}>
      <TouchableOpacity onPress={onCheck} style={$.measureUnitItemRow}>
        ...
        <Text style={$.measureUnitItemText}>
          {item.name} ({item.code})
        </Text>
      </TouchableOpacity>
