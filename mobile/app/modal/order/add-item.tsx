...
const AddItemModal = () => {
  ...
  const product = {
    ...
    buyingPrice: Number(params.buyingPrice) || 0,
  ...
  return (
    <ScreenModal modalStyle={$.modal}>
      <ScreenModal.Header
        title={orderItemId ? t("edit_item") : t("add_item")}
      />
      <ScreenModal.Body>
            ...
            <View style={$.inputGroup}>
              <Input
                ...
                onChangeText={(text) => {
                  ...
                  const maxPrice =
                    (product.price + totalVariantPrice) * quantity;
                  const maxAllowedDiscount =
                    maxPrice * (maxDiscountPercent / 100);

                  const numericValue = Number(text);
                  if (
                    text === "" ||
                    (numericValue >= 0 && numericValue <= maxAllowedDiscount)
                  ) {
                    setDiscount(text);
                  }
                }}
