...
const PaymentScreen = () => {
  ...
  const defaultMethod =
    paymentMethods.find((p) => p.code === "CASH")?.code ||
  ...
  const totalTax = items.reduce(
    (sum, item) => sum + (item.tax || 0) * item.quantity,
    0
  );
  ...
  const handlePay = () => {
    const transactionProducts: TransactionProductRequest[] = items.map(
      (item) => ({
            ...
            const receiptData = {
              ...
              items: items.map((item) => ({
                ...
                price: formatCurrency(
                  (item.sellingPrice +
                    (item.variants?.reduce((s, v) => s + (v.price || 0), 0) ||
                      0)) *
                    item.quantity
                ),
                variants:
