...
const PaymentScreen = () => {
  ...
  const defaultMethod =
    paymentMethods.find((p) => p.code === "CASH")?.code || paymentMethods[0]?.code || "CASH";
  ...
  const totalCommission = items.reduce(
    (sum, item) => sum + (item.commission || 0) * item.quantity,
    0,
  );
      ...
      },
      {
        onSuccess: (data) => {
          ...
          captureAnalyticsEvent("checkout_completed", {
            ...
            discountAmount: globalDiscount + totalItemDiscounts,
            taxAmount: totalTax,
            serviceFeeAmount: totalCommission,
          });
            ...
            const receiptData = {
              ...
              items: items.map((item) => ({
                ...
                price: formatCurrency(
                  (item.sellingPrice +
                    (item.variants?.reduce((s, v) => s + (v.price || 0), 0) || 0)) *
                    item.quantity,
                ),
                variants:
                  item.variants?.map((v) => ({
