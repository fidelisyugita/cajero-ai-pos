...
import { formatCurrency, parseCurrency, parseNumber } from "@/utils/Format";
...
const productSchema = z.object({
  ...
  buyingPrice: z.number().min(0, "Buying Price must be a positive number").optional(),
  category: z.object({
    code: z.string().min(2, "Category code must be at least 2 characters"),
    name: z.string().min(2, "Category name must be at least 2 characters"),
  }),

  commission: z.number().min(0, "Commission must be a positive number").optional(),
  ...
  productIngredients: z.array(
    z
      .object({
        ingredientId: z.string().min(2, "Ingredient ID must be at least 2 characters"),
...
const AddProduct = () => {
  ...
  useEffect(() => {
    if (productToEdit && categories) {
      const category = categories.find((c) => c.code === productToEdit.categoryCode);
  ...
  useEffect(() => {
    if (!isEditing && categories && categories.length > 0 && !getValues("category")) {
      setValue("category", {
  ...
  const onSaveAndAddMore = (data: ProductFormData) => processSubmit(data, false);
  ...
  const handleSelectCategoryPress = (category: { code: string; name: string }) => {
  ...
  const handleDelete = () => {
    Alert.alert(t("delete_product"), t("confirm_delete_product"), [
      {
        text: t("cancel"),
        style: "cancel",
      },
  ...
  const handleRestore = () => {
    Alert.alert(t("unhide_product"), t("confirm_unhide_product"), [
      {
        text: t("cancel"),
        style: "cancel",
      },
  ...
  return (
    <View style={$.container}>
      ...
      <KeyboardForm contentContainerStyle={$.scrollContent} style={$.flex}>
        <View style={$.section}>
          <FormSectionCard required style={$.imageSection} title={t("product_image")}>
            <Controller
              control={control}
              name="imageUrl"
              render={({ field: { value } }) => (
            ...
            <View style={$.categoryContainer}>
              {storeVariants && storeVariants.length > 0 && (
                <Text style={$.selectedCategoryText}>
                  {storeVariants.length} {storeVariants.length === 1 ? t("variant") : t("variants")}{" "}
                  {t("configured")}
                </Text>
              )}
              <Button
        ...
        <View style={$.section}>
          <FormSectionCard required title={t("product_information")}>
            ...
            <Controller
              ...
              render={({ field: { onChange, ref, value }, fieldState: { error } }) => (
                <Input
                  ...
                  returnKeyType="next"
                  size="lg"
                  value={value !== undefined ? formatCurrency(value || 0) : ""}
                />
              )}
            />
            <Controller
            ...
            <View style={$.taxToggleRow}>
              ...
              <Switch onValueChange={setIsTaxIncluded} value={isTaxIncluded} />
            </View>
                  ...
                  {value && value.length > 0 && (
                    <View style={$.ingredientListContainer}>
                      {value
                        .filter((item) => !!item)
                        .map((item) => (
                          <View key={item.ingredientId} style={$.ingredientCard}>
